...
class EchoBot(sleekxmpp.ClientXMPP):
    ...
    def checkExchange(self,msg,args):
        currency1 = args[1]
        currency2 = args[2]
        result = checkCurrency.checkExchange(currency1, currency2)
        #print message
        msg.reply( 
        """The exachge rate for %s to %s is %s
        %s %s
        bid: %s
        ask: %s
        """ % (currency1, currency2, result['exchangeRate'], result['day'], result['time'], result['bid'], result['ask'])).send()
        #msg.reply("%s vs %s" % (args[1],args[2])).send()
